...
func (m *Manager) GoToDefinition(ctx context.Context, uri string, line, character int) ([]Location, error) {
	if !m.IsRunning() {
...
func (m *Manager) FindReferences(ctx context.Context, uri string, line, character int, includeDeclaration bool) ([]Location, error) {
	if !m.IsRunning() {
		return nil, fmt.Errorf("gopls is not running")
	}
...
func (m *Manager) GetHover(ctx context.Context, uri string, line, character int) (*Hover, error) {
	if !m.IsRunning() {
		return nil, fmt.Errorf("gopls is not running")
	}
